...
const calcCRC32 = aribts.TsCrc32.calc;
...
class TSFilter extends stream.Duplex {
    constructor(options) {
        ...
        // aribts
        this._parser = new aribts.TsStream();
        // buffer
        this._packet = new Buffer(PACKET_SIZE);
        this._offset = -1;
        this._buffer = [];
        this._parses = [];
        ...
        this._closed = false;
        this._ready = true;
        this._providePids = null; // `null` to provides all
        this._parsePids = [];
        this._tsid = -1;
        this._patCRC = new Buffer(0);
        this._serviceIds = [];
        this._services = [];
        ...
        // stream options
        this.highWaterMark = _1.default.config.server.highWaterMark || 1024 * 1024 * 24;
        this._overflowTimeLimit = _1.default.config.server.overflowTimeLimit || 1000 * 30;
        ...
        if (options.parseEIT === true) {
            ...
            else if (status_1.default.epg[this._targetNetworkId] !== true) {
    ...
    _write(chunk, encoding, callback) {
        ...
        if (this._readableState.length > this.highWaterMark) {
            ...
            ++status_1.default.errorCount.bufferOverflow;
            return;
        }
        if (this._overflowTimer !== null) {
    ...
    _processPacket(packet, mustCopy) {
        ...
        else if (((pid === 0x12 || pid === 0x29) && (this._parseEIT === true || this._provideEventId !== null)) ||
            pid === 0x14 ||
    ...
    _onPAT(pid, data) {
        ...
        for (let i = 0, l = data.programs.length; i < l; i++) {
            let item;
            const id = data.programs[i].program_number;
            if (id === 0) {
            ...
            if (this._targetNetworkId === null) {
                ...
                item = _1.default.service.get(this._targetNetworkId, id);
            }
            log.debug("TSFilter detected PMT PID=%d as serviceId=%d (%s)", data.programs[i].program_map_PID, id, item ? item.name : "unregistered");
            ...
            if (this._parseEIT === true && item) {
                _1.default.service.findByNetworkId(this._targetNetworkId).forEach(service => {
    ...
    _onCDT(pid, data) {
        if (data.data_type === 0x01) {
            ...
            _1.default.service.findByNetworkIdWithLogoId(data.original_network_id, dataModule.logo_id).forEach(service => {
