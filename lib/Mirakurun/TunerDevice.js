...
class TunerDevice extends events.EventEmitter {
    constructor(_index, _config) {
        ...
        this._exited = false;
        this._closing = false;
        Event_1.default.emit("tuner", "create", this.export());
    ...
    startStream(user, stream, channel) {
        ...
        if (this._config.types.indexOf(channel.type) === -1) {
            return Promise.reject(new Error(util.format("TunerDevice#%d is not supported for channel type `%s`", this._index, channel.type)));
        }
        if (this._stream) {
    ...
    _updated() {
        Event_1.default.emit("tuner", "update", this.export());
    }
}
